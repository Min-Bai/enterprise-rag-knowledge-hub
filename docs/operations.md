...
- 客户端注册和密码重置均进入管理端“账户审批”，批准后才会创建账号或生成一次性重置链接。使用 `scripts/create_admin.py` 创建首个管理员；确认身份后使用 `scripts/promote_user.py` 提升已有用户。遗留 `ALLOW_SELF_REGISTRATION` 必须保持 `false`。
