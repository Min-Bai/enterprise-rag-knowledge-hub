...
`Build and Deploy Enterprise RAG` deploys tags in the form `sha-<full-commit-sha>`. Manual runs are accepted only from `main`. The hosted build job releases its cache, then streams the CPU-only API image and web image into separate compressed archives in a one-day artifact. The runner transfers both archives and the VM streams them into Docker before starting Compose.
