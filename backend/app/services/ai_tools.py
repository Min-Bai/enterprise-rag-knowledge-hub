...
def call_model(*, db: Session, system: str, user: str, max_tokens: int = 800, json_mode: bool = False) -> str:
...
def summarize_knowledge_base(*, knowledge_base_id: int, user: UserORM, db: Session) -> tuple[str, list[SourceItem]]:
    ...
    answer = call_model(db=db, system="你是企业知识库摘要助手。只能依据提供的文档内容总结；证据不足时明确说明，不得补造事实。使用中文，按主题、关键事实、结论输出。", user=_context(hits))
...
def extract_information(*, knowledge_base_id: int, user: UserORM, db: Session) -> tuple[dict[str, list[str]], list[SourceItem]]:
    ...
    raw = call_model(db=db, system="你是企业文档信息抽取器。仅依据文档内容，返回严格 JSON：{\"people\": [\"人名\"], \"amounts\": [\"金额及原文单位\"], \"clauses\": [\"条款原文或准确摘要\"]}。不确定的字段返回空数组。", user=_context(hits), max_tokens=1000, json_mode=True)
    try:
        data = json.loads(raw)
