...
def send_password_reset_email(*, recipient_email: str, reset_token: str, expires_at: datetime) -> None:
    ...
    message.set_content(
        ...
        "该链接只能使用一次。如果这不是您本人提交的申请，请立即联系系统管理员。\n",
        charset="utf-8",
    )

    try:
        smtp_class = smtplib.SMTP_SSL if MAIL_SMTP_SSL else smtplib.SMTP
        with smtp_class(MAIL_SMTP_HOST, MAIL_SMTP_PORT, timeout=MAIL_TIMEOUT_SECONDS) as client:
            client.ehlo()
            if MAIL_SMTP_STARTTLS and not MAIL_SMTP_SSL:
