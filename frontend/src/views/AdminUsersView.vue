...
import { createAdminUser, createPasswordResetLink, createUserInvitation, deleteAdminUser, deleteUserInvitations, getAdminUsers, getUserInvitations, revokeUserInvitation, setUserRole, setUserStatus, type UserInvitation } from "../api/admin";
...
const selectedInvitations = ref<UserInvitation[]>([]);
...
async function removeInvitations() {
  if (!auth.token || selectedInvitations.value.length === 0) return;
  try {
    await ElMessageBox.confirm(`将删除选中的 ${selectedInvitations.value.length} 条邀请记录，已接受的邀请也会被清理。确认继续吗？`, "批量删除邀请记录", { type: "warning" });
    saving.value = true;
    const ids = selectedInvitations.value.map((item) => item.id);
    await deleteUserInvitations(auth.token, ids);
    invitations.value = invitations.value.filter((item) => !ids.includes(item.id));
    selectedInvitations.value = [];
    ElMessage.success("邀请记录已批量删除");
  } catch (caught) {
    if (caught !== "cancel" && caught !== "close") ElMessage.error(caught instanceof Error ? caught.message : "批量删除邀请记录失败。");
  } finally { saving.value = false; }
}
