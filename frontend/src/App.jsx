...
function App() {
  ...
  const selectedKnowledgeBase = knowledgeBases.find((item) => String(item.id) === selectedKnowledgeBaseId)
  ...
  const canManageDocuments = ['owner', 'editor'].includes(selectedKnowledgeBaseRole)
  ...
  async function handleDeleteConversation() {
    ...
    } catch (error) { setDocumentAnswerError(error.message) } finally { setDeletingConversationId(null) }
  }
  ...
  return <main className="app-shell">
    ...
    {!accessToken ? <LoginForm onLogin={handleLogin} /> : <section className="document-answer-panel" aria-label="Knowledge base documents">
      ...
      {auditLogs.length > 0 && <section className="knowledge-base-members"><h2>Audit log</h2><ul className="document-list">{auditLogs.map((event) => <li key={event.id} className="document-list-item"><div><strong>{event.action}</strong><span className="audit-actor">{event.actor_username} · {event.target_type}{event.target_id ? ` #${event.target_id}` : ''}</span>{event.details && <small className="audit-details">{Object.entries(event.details).map(([key, value]) => `${key}: ${value}`).join(' · ')}</small>}</div><time dateTime={event.created_at}>{new Date(event.created_at).toLocaleString()}</time></li>)}</ul></section>}
