...
export function approvePasswordResetRequest(token: string, id: string, expiresInHours: number) { return adminRequest<{ expires_at: string; reset_token: string }>(`/password-reset-requests/${id}/approve`, { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify({ expires_in_hours: expiresInHours }) }); }
