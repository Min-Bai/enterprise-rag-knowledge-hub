...
export async function answerDocument(
  accessToken,
  documentId,
  question,
  conversationId,
...
export async function streamDocumentAnswer(
  ...
  if (conversationId) body.conversation_id = Number(conversationId);
  return streamAnswer(
    `${API_PREFIX}/ai/document-answer/stream`,
    accessToken,
    body,
    handlers,
  );
...
export async function streamKnowledgeBaseAnswer(
  ...
  return streamAnswer(
    `${API_PREFIX}/ai/knowledge-base-answer/stream`,
    accessToken,
    body,
    handlers,
  );
...
export async function deleteConversation(accessToken, conversationId) {
  const response = await fetch(
    `${API_PREFIX}/ai/conversations/${conversationId}`,
...
export async function getKnowledgeBases(accessToken) {
  return readJson(
    await fetch(`${API_PREFIX}/knowledge-bases`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    }),
...
export async function deleteKnowledgeBase(accessToken, knowledgeBaseId) {
  const response = await fetch(
    `${API_PREFIX}/knowledge-bases/${knowledgeBaseId}`,
    { method: "DELETE", headers: { Authorization: `Bearer ${accessToken}` } },
  );
  if (response.status !== 204)
    await readJson(response, "Failed to delete knowledge base");
}
...
export async function updateKnowledgeBase(
  ...
  return readJson(
    await fetch(`${API_PREFIX}/knowledge-bases/${knowledgeBaseId}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
...
export async function getKnowledgeBaseMembers(accessToken, knowledgeBaseId) {
  return readJson(
    await fetch(`${API_PREFIX}/knowledge-bases/${knowledgeBaseId}/members`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    }),
    "Failed to load knowledge base members",
  );
}
...
export async function removeKnowledgeBaseMember(
  accessToken,
  knowledgeBaseId,
  userId,
) {
  const response = await fetch(
    `${API_PREFIX}/knowledge-bases/${knowledgeBaseId}/members/${userId}`,
    { method: "DELETE", headers: { Authorization: `Bearer ${accessToken}` } },
  );
  if (response.status !== 204)
...
export async function getKnowledgeBaseFeedbackSummary(
  accessToken,
  knowledgeBaseId,
) {
  return readJson(
    await fetch(
      `${API_PREFIX}/knowledge-bases/${knowledgeBaseId}/feedback-summary`,
      { headers: { Authorization: `Bearer ${accessToken}` } },
    ),
    "Failed to load feedback quality",
  );
}
...
export async function submitAnswerFeedback(
  ...
  return readJson(
    await fetch(`${API_PREFIX}/ai/answer-messages/${messageId}/feedback`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
...
export async function getMyDocuments(
  ...
  return readJson(
    await fetch(`${API_PREFIX}/documents?${params}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    }),
...
export async function searchDocuments(
  ...
  return readJson(
    await fetch(`${API_PREFIX}/documents/search?${params}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
...
export async function retryDocument(accessToken, documentId) {
  return readJson(
    await fetch(`${API_PREFIX}/documents/${documentId}/retry`, {
      method: "POST",
      headers: { Authorization: `Bearer ${accessToken}` },
    }),
...
export async function reindexDocument(accessToken, documentId) {
  return readJson(
    await fetch(`${API_PREFIX}/documents/${documentId}/reindex`, {
      method: "POST",
      headers: { Authorization: `Bearer ${accessToken}` },
    }),
...
export async function updateDocumentTags(accessToken, documentId, tags) {
  return readJson(
    await fetch(`${API_PREFIX}/documents/${documentId}/tags`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
...
export async function downloadDocument(accessToken, selectedDocument) {
  const response = await fetch(
    `${API_PREFIX}/documents/${selectedDocument.id}/download`,
    { headers: { Authorization: `Bearer ${accessToken}` } },
  );
